...
const HeroCarousel = () => {
  ...
  return <section className="relative h-auto min-h-[300px] sm:min-h-[400px] md:min-h-[500px] lg:h-[700px] overflow-hidden mt-20">
      {slides.map((slide, index) => <div key={index} className={`${index === 0 ? 'relative' : 'absolute inset-0'} transition-opacity duration-1000 ${index === currentSlide ? "opacity-100 z-10" : "opacity-0 z-0"}`}>
